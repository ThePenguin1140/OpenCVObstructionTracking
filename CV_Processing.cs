...
namespace ShaprCVTest
{
    class PreProcessing
    {
                ...
                if (!CV_Program.TrackCups || GetMinDistance(box) < 150)
                {
                    if ((int) BoxSplits[i] == 1)
                    {
                        ...
                        if (frame != null && CV_Program.TrackCups)
                            CvInvoke.PutText(frame, "[" + (GetCupNum(box) + 1) + "]",
                                new System.Drawing.Point(box.Location.X + 5, box.Location.Y - 10),
                                FontFace.HersheyPlain, 1.25, new MCvScalar(255, 0, 255), 2);
                    }
                    else if ((int) BoxSplits[i] == 2)
                    {
                        ...
                        if (frame != null && CV_Program.TrackCups)
                            CvInvoke.PutText(frame, "[" + (GetCupNum(box2) + 1) + "]",
                                new System.Drawing.Point(box2.Location.X + 5, box2.Location.Y - 10),
                                FontFace.HersheyPlain, 1.25, new MCvScalar(255, 0, 255), 2);
                        if (frame != null && CV_Program.TrackCups)
                            CvInvoke.PutText(frame, "[" + (GetCupNum(box1) + 1) + "]",
                                new System.Drawing.Point(box1.Location.X + 5, box1.Location.Y - 10),
                                FontFace.HersheyPlain, 1.25, new MCvScalar(255, 0, 255), 2);
                    }
                }
            }
        ...
        public static float GetDistance(Rectangle rect1, Rectangle rect2)
        {
            ...
            float distance =
                (float) Math.Sqrt((horizontalDistance * horizontalDistance) + (verticalDistance * verticalDistance));
